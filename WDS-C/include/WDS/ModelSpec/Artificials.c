...
#define at_(r,c) (bRowMajor) ? ((r+nrvRowOffset)*nrvColumnCount+c+nrvColumnOffset) : ((r+nrvRowOffset)+(c+nrvColumnOffset)*nrvRowCount)
    ...
    int fArtificials_Numeric(double* SourceValue  // possibly a vector
            ...
            , size_t nCleanLimits
            , double* rv
            , size_t nrv            // just the column count expected to be returned with (Treatment, nCritVals)
            // This could be determined within, but generally has already be calculated to allocate
            // space for rv.
            , size_t nrvRowCount    // generally >= nSourceValueRowCount, but must be >= nSourceValueRowCount+nrvRowOffset
            , size_t nrvColumnCount // generally = nArtificialsCount, but must be >= nArtificialsCount+nrvColumnOffset
            , size_t nrvRowOffset // generally = 0, but can be used to imbed result into a system matrix
            , size_t nrvColumnOffset // generally = 0, but can be used to imbed result into a system matrix
            , bool bRowMajor
            ) {
		...
		if (nrv > NARTVALS_MAX) return -1;

        if (nSourceValueRowCount<1 || nrvRowOffset<0 || nrvRowOffset>nrvRowCount) return -1;
        if (nrvRowCount<nSourceValueRowCount+nrvRowOffset) return -1;
        if (nrvColumnOffset<0 || nrvColumnOffset+nrv>nrvColumnCount) return -1;
        if (nrvRowCount<nSourceValueRowCount) return -1;
...
*/
        ...
        if (eTreatment_bIn(3, Treatment, e_Categorical, e_CategoricalNumeric)) { //then
            return -1;
            //reserved for fArtificials_Categorical
            /*
            ...
            if (eTreatment_bIn(5, Treatment, e_Hats, e_iHats, e_BSplineOrder2, e_BSplineOrder3)) { // then
            ...
            if (eTreatment_bIn(4, Treatment, e_iHats, e_BSplineOrder2, e_BSplineOrder3)) { // then
            ...
            if (Treatment == e_BSplineOrder3) { // then
		...
		for (r = 0; r < nrows; r++) {
			...
			default:
				...
				if (bIsMissing) {
					...
					if (Treatment == e_CodedMissings) { // then
						...
						if (Treatment == e_iHats) { //then
							...
							rv[at_(r, ia)] = x - CriticalValues[0];
						...
						for (ia = ia + 1; ia < nrv; ia++)
							rv[at_(r, ia)] = 0.0;

					}
					else if (x >= CriticalValues[nCritValsM1] - eps) { // then
						...
						if (Treatment == e_iHats) { // then
							rv[at_(r, ia)] = x - CriticalValues[i] + dCVs[i - 1] / 2.0;
							for (jM1 = 0, j = 1, ia = 2; j < nCritVals; jM1++, j++, ia++)
								rv[at_(r, ia)] += d2CVsdiv2[jM1];

							j = 0;
							ia = 1;
							rv[at_(r, ia)] += dCVs[j] / 2.0;
						}
						else {
						...
						case e_Hats:
							...
							rv[at_(r, ia + 1)] = tempdouble;
							rv[at_(r, ia)] = (1.0 - tempdouble);
						...
						case e_iHats:
							...
							rv[at_(r, iaP1)] = tempdouble;
							rv[at_(r, ia)] = (x - CriticalValues[i] - tempdouble);

							for (ii = 1; ii < i; ii++) {
								iia = ii + 1;
								iiaP1 = iia + 1;
								rv[at_(r, iiaP1)] += dCVsdiv2[ii];
								rv[at_(r, iia)] += dCVsdiv2[ii];
							}
