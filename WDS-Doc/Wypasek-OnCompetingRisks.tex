...
%can be run with ../WDS-General/util/llatex Wypasek-OnCompetingRisks.tex --doublerun --clean
...
%\def\thisfirm{NA}
\def\thisdate{NA}
\def\thiscopyright{Copyright 2022-2026}
...
\end{center}
Many of the formulas included herein are also from old writeups.
There may be some changes in notation until this document gets finalized (if ever). 
...
%Here, we will take $\eta(i,j) \in \{0, 1\}$ for simplicity and for explicit integration of strata and Breslow ties, for an uncensored
%subject, $sbj_i$, 
%let $S_i$ denote its relative survival set.  In other words, the relative survival set for a given subject is the set of all subjects that are both {\em at-risk} 
%at time $t_{sbj_i}$ and within the same strata.    We will let $\eta_i$ be used as a weight for subject $i$.
