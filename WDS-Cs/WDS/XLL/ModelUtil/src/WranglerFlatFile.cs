...
import static WDS.Util.PathAndName;
import static WDS.Util.MatchingNullityAndValueEquals;
import static WDS.Util.bIn;
    ...
    public class WranglerFlatFile
    {
        ...
        //throws WDSException
        {
            ...
            try {
                ...
                aPathAndName = WDS.Util.PathAndName(this.Path, this.FileName);
                /* Java >>> *
        ...
        //throws WDSException
        {
            try {
                ...
                {
                    aPathAndName = WDS.Util.PathAndName(aPath, aFileName);
                ...
                while ( true ) {
                    ...
                    if ( nIncomingFields > nInputMap )
                        //Java throw new WDSException(String.format("Error reading from delimited file, row %d has insufficient columns, stopping input!\n", row + 1));
                        //C#
                        throw new WDSException("Error reading from delimited file, row " + (row + 1) + " has insufficient columns, stopping input!\n");
                    ...
                    for ( j = 0 ; j < nInputMap ; j++ ) {
                        ...
                        if ( s == null || s.isEmpty() ) {
                            ...
                            //C#
                            String lv = WDS.Util.CleanAsToken(inputLine.get(j));
                            inputRow_orig[j] = lv;
                            if ( aInputRecordSetMD.Column[j].hasMapKey() )
                                inputRow.put(aInputRecordSetMD.Column[j].MapKey, lv);
                        }
                ...
                throw new WDSException("Error reading from delimited file:", e);
            }
            catch ( Exception e ) {
                throw new WDSException("Error reading from delimited file:", e);
            }
        ...
        //throws WDSException
        {
            ...
            if ( aOutputRecordSetMD.Type.bIn(RecordSetMDEnums.eType.CSV, RecordSetMDEnums.eType.TXT, RecordSetMDEnums.eType.Dlm) ) {
                try {
                    ...
                    String aPathAndName = null;
                    aPathAndName = WDS.Util.PathAndName(aOutputRecordSetMD.FileMatter.Path, aOutputRecordSetMD.FileMatter.FileName);
                    ...
                    throw new WDSException("Error in WranglerFlatFile.mWriteRecordSet", e);
                }
