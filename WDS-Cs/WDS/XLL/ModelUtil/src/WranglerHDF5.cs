...
import static com.WDataSci.JniPMML.Util.PathAndName;
...
namespace WDS.Wrangler
{
    ...
    public class WranglerHDF5
    {
        ...
        public void Dispose()
            throws WDSException
            {
                try {
                    ...
                    throw new WDSException("Error finalizing Wrangler for HDF5", e);
                }
            }
        ...
        //throws WDSException
        {
            //Java synchronized (aRecordSetMD) {
            try {

                int j = -1;
                //Java int ii=-1,jj=-1,kk=-1;
                //C# 
                //uint ii = 0, jj = 0;
                uint jj = 0;
                ...
                try {
                    ...
                    //Java aRecordSetMD.HDF5Matter.File = new hdf.object.h5.H5File(aPathAndName, hdf.object.h5.H5File.READ);
                    //Java aRecordSetMD.HDF5Matter.File.open();
                    //C#
                    aRecordSetMD.HDF5Matter.File = H5F.open(aPathAndName, H5F.ACC_RDONLY);
                }
                /* Java >>> *
                   catch (Throwable e) 
                /* <<< Java */
                ...
                try {
                    //Java aRecordSetMD.HDF5Matter.CompoundDS = new H5CompoundDS(aRecordSetMD.HDF5Matter.File, aRecordSetMD.HDF5Matter.DSName, "/");
                    //Java aRecordSetMD.HDF5Matter.CompoundDS.read();
                    //C# 
                    aRecordSetMD.HDF5Matter.CompoundDS = H5D.open(aRecordSetMD.HDF5Matter.File, hDSPath);
                }
                catch ( Exception e ) {
                ...
                if ( hClass != H5T.class_t.COMPOUND )
                    throw new WDSException("Data set " + hDSPath + " in HDF5 file " + aRecordSetMD.FileMatter.FileName + " is not a compound type!");
                ...
                for ( jj = 0 ; jj < hnColumns ; jj++ ) {
                    ...
                    if ( !cm.DTyp.bIn(FieldMDEnums.eDTyp.Dte, FieldMDEnums.eDTyp.DTm) ) {
        ...
        //throws WDSException, Exception
        {
            int nColumns = aRecordSetMD.nColumns();
            for ( int jj = 0 ; jj < nColumns ; jj++ ) {
                if ( aRecordSetMD.Column[jj].DTyp.bIn(FieldMDEnums.eDTyp.Str) )
                    aRecordSetMD.Column[jj].DTyp = FieldMDEnums.eDTyp.Str;
                aRecordSetMD.Column[jj].HDF5DataType = new HDF5DataType(aRecordSetMD.Column[jj].DTyp, aRecordSetMD.ModeMatter.OutputMaxStringLength, Default.anyVLenWrite);
            }
        }

        //Java public void mReadRecordSet(RecordSetMD aInputRecordSetMD, RecordSet aInputRecordSet, PrintWriter pw)
        //C#
        ...
        //throws WDSException, Exception
        {
            ...
            try {
                ...
                //Java int rank = aInputRecordSetMD.HDF5Matter.CompoundDS.getRank();
                ...
                //Java int nRows = (int) aInputRecordSetMD.HDF5Matter.CompoundDS.getDims()[0];
                //C#
                int nRows = (int) sdim[0];
                            ...
                            switch ( aInputRecordSetMD.Column[j].DTyp ) {
                                ...
                                case FieldMDEnums.eDTyp.Dte: {
                                    if ( aInputRecordSetMD.Column[i].ExternalDTyp.isNumeric() ) {
                                        ...
                                        if ( aInputRecordSetMD.Column[j].hasMapKey() )
                                            inputRow.put(aInputRecordSetMD.Column[j].MapKey, lv);
                                        throw new WDSException("Error Date and DateTime conversions not implemented yet");
                                    }
                                }
                                break;
                                case FieldMDEnums.eDTyp.DTm: {
                                ...
                                case FieldMDEnums.eDTyp.Byt:
                                default:
                                    throw new WDSException("Error, unhandled DTyp in conversion from HDF5");
                            }
                        }
        ...
        //Java public void mWriteRecordSet(RecordSetMD aOutputRecordSetMD, RecordSet aOutputRecordSet, RecordSetMD aInputRecordSetMD, RecordSet aInputRecordSet)
        //C#
        ...
        //throws WDSException
        {
            ...
            try {
                ...
                String aPathAndName = WDS.Util.PathAndName(aOutputRecordSetMD.FileMatter.Path, aOutputRecordSetMD.FileMatter.FileName);
                ...
                //Java @Nullable
                //Java hdf.object.Datatype[] memberDatatypes = new hdf.object.Datatype[nColumns];
                //C#
                System.Int64[] memberDatatypes = new System.Int64[nColumns];
                int[] memberSizes = new int[nColumns];
                //Java @Nullable
                //Java Vector<Object> data = new Vector<>();
                //C#
                System.Collections.Generic.List<Object> data = new System.Collections.Generic.List<Object>(nColumns);

                //Java @Nullable
                //Java hdf.object.Group pgroup = (hdf.object.Group) outHDF5.get("/");
                //C#
                ...
                jj = 0;
                if ( aOutputRecordSetMD.ModeMatter.bRepeatInputFields ) {
                    //Java ArrayList<Object> data_orig = null;
                    //C#
                    System.Collections.Generic.List<Object> data_orig = null;
                    if ( aInputRecordSetMD.Type.bIn(RecordSetMDEnums.eType.HDF5) ) {
                        //Java data_orig = (ArrayList<Object>) aInputRecordSetMD.HDF5Matter.CompoundDS.getData();
                        //C#
                        //??????? data_orig = (System.Collections.Generic.List<Object>) aInputRecordSetMD.HDF5Matter.CompoundDS.getData();
                    }
                    else {
                        //Java data_orig = new ArrayList<>(nInputColumns);
                        //C#
                        data_orig = new System.Collections.Generic.List<Object>(nInputColumns);
                    }
                    for ( j = 0 ; j < nInputColumns ; j++ ) {
                        ...
                        if ( aInputRecordSetMD.Type.bIn(RecordSetMDEnums.eType.HDF5) ) {
                            //Java data.add(j, data_orig.get(j));
                            //C#
                            data[j] = data_orig[j];
                        }
                        else {
                ...
                for ( k = 0, j = jj ; k < nResultColumns ; k++, j++ ) {
                    ...
                    switch ( aOutputRecordSetMD.Column[j].DTyp ) {
                        ...
                        //case DTm:
                        case FieldMDEnums.eDTyp.DTm:
                            //Java data.add(new double[nRows]);
                            //C#
                            data[j] = new double[nRows];
                            break;
                        //case Lng:
                        case FieldMDEnums.eDTyp.Lng:
                            //Java data.add(new long[nRows]);
                            //C#
                            data[j] = new long[nRows];
                            break;
                        //case Bln:
                        case FieldMDEnums.eDTyp.Bln:
                        //case Int:
                        case FieldMDEnums.eDTyp.Int:
                            //Java data.add(new int[nRows]);
                            //C#
                            data[j] = new int[nRows];
                            break;
                        //case VLS:
                        case FieldMDEnums.eDTyp.VLS:
                        //case Str:
                        case FieldMDEnums.eDTyp.Str:
                            //Java data.add(new String[nRows]);
                            //C#
                            data[j] = new String[nRows];
                            break;
                        //case Byt:
                        case FieldMDEnums.eDTyp.Byt:
                        default:
                ...
                for ( i = 0 ; i < nRows ; i++ ) {
                    for ( k = 0, j = nInputColumns ; k < nResultColumns ; k++, j++ ) {
                        obj = aOutputRecordSet.Records.get(i).get(aOutputRecordSetMD.Column[j].MappedKey());
                        switch ( aOutputRecordSetMD.Column[j].DTyp ) {
                            //case Dte:
                            case FieldMDEnums.eDTyp.Dte:
                            //case DTm:
                            ...
                            case FieldMDEnums.eDTyp.Lng:
                                if ( obj == null )
                                    //Java ((long[]) data.get(j))[i] = Long.MIN_VALUE;
                                    //C#
                                    ((long[]) data.get(j))[i] = long.MinValue;
                                else
                            ...
                            case FieldMDEnums.eDTyp.Int:
                                if ( obj == null )
                                    //Java ((int[]) data.get(j))[i] = Integer.MIN_VALUE;
                                    //C#
                                    ((int[]) data.get(j))[i] = int.MinValue;
                                else
                ...
                throw new WDSException("Error writing HDF5", e);
            }
                ...
                )
        {
            try {
                ...
                if ( nColumns != aRecordSetMD.nColumns() )
                    throw new WDSException("Error, Excel ListObject #columns does not match RecordSetMD #columns");


                if ( this.DSName == null || this.DSName.isEmpty() )
                    this.DSName = "RecordSet";

                String aPathAndName = WDS.Util.PathAndName(aRecordSetMD.FileMatter.Path, aRecordSetMD.FileMatter.FileName);
                        ...
                        try {
                            ...
                            for ( int j = 0 ; j < nColumns ; j++ ) {
                                ...
                                try {
                                    ...
                                    throw new WDSException("Error writing attributes to " + aRecordSetMD.HDF5Matter.DSName, e);
                                }
                ...
                throw new WDSException("Error in writing output map to DBB", e);
            }
        }
        /* <<< C# */
        ...
        public HDF5DataType new_HDF5DataType(FieldMDEnums.eDTyp DTyp, long nStringMaxLength, Boolean anyVLen)
        //throws WDSException, Exception
        {
        ...
        public class HDF5DataType
        {
            ...
            //Java public hdf.object.Datatype data = null;
            //C#
            public System.Int64 data = -1;

            //C#
            public HDF5DataType(System.Int64 hclass, int hlength, int horder, int hsign)
            //Java public HDF5DataType(int hclass, int hlength, int horder, int hsign) 
            {
                //Java this.data = new hdf.object.h5.H5Datatype(hclass, hlength, horder, hsign);
                /* C# >>> */
                ...
                if ( bIn((long) hclass, H5T.C_S1, H5T.NATIVE_CHAR) ) {
                    ...
                    throw new WDSException("Error HDF5 output datatype not handled");
                /* <<< C# */
            }
            ...
            public HDF5DataType(FieldMDEnums.eDTyp DTyp, long nStringMaxLength, Boolean anyVLen)
            //throws WDSException
            {
                switch ( DTyp ) {
                    //case Dbl:
                    ...
                    //case DTm:
                    case FieldMDEnums.eDTyp.DTm:
                        //Java this.data = new hdf.object.h5.H5Datatype(hdf.object.Datatype.CLASS_FLOAT, 8, -1, -1);
                        //C#
                        this.data = H5T.NATIVE_DOUBLE;
                        return;
                    //case Lng:
                    case FieldMDEnums.eDTyp.Lng:
                        //Java this.data = new hdf.object.h5.H5Datatype(hdf.object.Datatype.CLASS_INTEGER, 8, -1, -1);
                        //C#
                        this.data = H5T.NATIVE_LONG;
                        return;
                    //case Int:
                    case FieldMDEnums.eDTyp.Int:
                        //Java this.data = new hdf.object.h5.H5Datatype(hdf.object.Datatype.CLASS_INTEGER, 4, -1, -1);
                        //C#
                        this.data = H5T.NATIVE_INT32;
                        return;
                    //case VLS:
                    case FieldMDEnums.eDTyp.VLS:
                        /* Java >>> *
                    ...
                    //case Str:
                    case FieldMDEnums.eDTyp.Str:
                        //Java this.data = new hdf.object.h5.H5Datatype(hdf.object.Datatype.CLASS_STRING, (int) nStringMaxLength, -1, -1);
                        /* C# >>> */
                        System.Int64 hStrType = H5T.copy(H5T.C_S1);
                    ...
                    //case Bln:
                    case FieldMDEnums.eDTyp.Bln:
                        //Java this.data = new hdf.object.h5.H5Datatype(hdf.object.Datatype.CLASS_INTEGER, 1, -1, -1);
                        //C#
                        this.data = H5T.NATIVE_INT32;
                        return;
                    default:
                        throw new WDSException("Error, un-implemented OutputColumn DataType !" + DTyp.ToString());
                }
            }

            public FieldMDEnums.eDTyp eDTyp()
            {
                /* Java >>> *
