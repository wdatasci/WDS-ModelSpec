...
//using static WDS.JavaLikeExtensions;
    ...
    public static partial class Util
    {
        ...
        //throws WDSException
        {
            ...
            if ( arg.endsWith("Set") ) {
                ...
                throw new WDSException("RecordSingleName: Cannot identify usual name for a single row from RecordSet name (should end in \"Set\" or \"s\")");
            return rv;
        }
        ...
        //throws WDSException
        {
            if ( xInputSchema == null )
                throw new WDSException("Invalid XMLSchema input to RecordSetElementName used to extract value when not used as an input");
            /* Java >>> *
            ...
            try {
                ...
                if ( xnl.getLength() == 0 )
                    throw new WDSException("Without RecordSetElementName provided, cannot find any 'element' off of root in InputSchema");
                if ( xnl.getLength() > 1 )
                    throw new WDSException("Without RecordSetElementName provided, cannot find a single 'element' off of root in InputSchema");
                return ((Element) xnl.item(0)).getAttribute("name");
            } catch (Exception e) {
                throw new WDSException("Without RecordSetElementName provided, cannot find single 'element' off of root in InputSchema\n" + e.getStackTrace().toString());
            }
            /* <<< Java */
            /* C# >>> */
